...
In the optional reference topology, a public HTTPS CONNECT edge terminates TLS
on TCP/8444 with an allowlisted SNI/certificate mapping
...
loopback backend, while the external TLS edge remains an independently reviewed
configuration transaction. Ordinary HTTP `proxy_pass` is not equivalent to
this stream contract and is not supported.

The HTTPS listener supports two fixed, independently provisioned identities.
`HTTPS_USER_1`/`HTTPS_PASSWORD_1` and
`HTTPS_USER_2`/`HTTPS_PASSWORD_2` represent separate clients. Both pairs are
required systemd credentials. A release that contains the second identity must
not be activated until its root-only files exist; the renderer otherwise fails
closed before replacing the running process. The identities must never reuse a
password.
