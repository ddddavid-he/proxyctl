...
// gateway TCP 8443/18444/7890, gateway TCP+UDP 17894, and egress UDP 443.
// Public HTTPS CONNECT TCP/443 is owned by the external OpenResty TLS edge;
// Mihomo reserves only its fixed loopback backend 18444. Bind scope
// (loopback-only for gateway 18444/7890/17894) is a static config policy declared here; it is separate
...
func roleReservations(role string) []reservation {
	...
	case "gateway":
		return []reservation{
			...
			{proto: "tcp", port: 17894, loopbackOnly: true, label: "17894/tcp (loopback ikev2 tproxy)"},
			{proto: "udp", port: 17894, loopbackOnly: true, label: "17894/udp (loopback ikev2 tproxy)"},
		}
	case "egress":
		return []reservation{
